...
/** Small row of export buttons shown beneath each event. */
function EventExportButtons({ event }) {
  return (
    <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
      <button
        ...
        title="Download .ics file for Apple / Outlook / any calendar"
      >
        ⬇️ .ics
      </button>
      <a
...
export default function Calendar() {
  ...
  function openAdd(date) {
    setEditing(null);
    setForm({ ...blankEvent(), date: date || new Date().toISOString().split('T')[0] });
    setShowForm(true);
  }
  function openEdit(e) {
    setEditing(e.id);
    setForm({ title: e.title, date: e.date, time: e.time || '', type: e.type, assignedTo: e.assignedTo || '', notes: e.notes || '' });
    setShowForm(true);
  }
  ...
  function save() {
    ...
    if (editing) {
      setState(prev => ({
        ...prev,
        calendar: prev.calendar.map(e => e.id === editing ? { ...e, ...form, title: form.title.trim() } : e),
      }));
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Events, appointments, and reminders</p>
        </div>
        ...
        <div className="card">
          ...
          <div className="cal-grid">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  key={d}
                  className={`cal-day ${ds === todayStr ? 'today' : ''} ${ds === selectedDate ? 'today' : ''} ${hasEvents ? 'has-events' : ''}`}
          ...
          {selectedDate && (
            <div style={{ marginTop: 16 }}>
              ...
              {dayEvents.length === 0
                ? <p style={{ color: 'var(--text-light)', fontSize: '0.9rem' }}>No events. Click + Add Event.</p>
                : dayEvents.map(ev => (
        ...
        <>
          {events.length === 0 && (
            <div className="empty-state card">
              ...
              <h3>No events yet</h3>
              <p>Add events, appointments, and reminders.</p>
              <button className="btn btn-primary" onClick={() => openAdd()}>+ Add First Event</button>
            </div>
          )}
        ...
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save} disabled={!form.title.trim() || !form.date}>
      ...
      {confirmDelete && (
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title">🗑️ Delete Event?</div>
            <p>Delete <strong>{events.find(e => e.id === confirmDelete)?.title}</strong>?</p>
            <div className="modal-footer">
